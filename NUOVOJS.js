...
function CercaLavoro (posizioneLAV, posizioneGEO){
const titoloMinuscolo = posizioneLAV.toLowerCase();
const locationMinuscolo = posizioneGEO.toLowerCase();
let result = []; 
    for(let i = 0; i < jobs.length; i++ ){
        let LAV = jobs[i].title.toLowerCase()
        let POS = jobs[i].location.toLowerCase()
            if(jobs[i].title.toLowerCase().includes(titoloMinuscolo) && jobs[i].location.toLowerCase().includes(locationMinuscolo)){
            result.push(jobs[i])
        } 
    }
return result;  
...
document.getElementById("Cerca").addEventListener("click", function(){
    const posizioneLAV = getElementById("lavoro").value; 
    const posizioneGEO = getElementById("geografia").value;
    const CercaRisultati = CercaLavoro(posizioneLAV, posizioneGEO); 
    const ListaRisultati = document.getElementById("ListaRisultati"); 
    ListaRisultati.innerHTML = ''; 
        if(CercaRisultati > 0){
            CercaRisultati.results.forEach(i => {
             const li = document.createElement('li'); 
             li.textContent = jobs.title + "-" + jobs.location; 
             ListaRisultati.appendChild(li); 
            });    
        } else {
            const li = document.createElement('li'); 
            li.textContent = 'Nessun lavoro trovato'; 
            ListaRisultati.appendChild(li);
        }
    console.log (CercaRisultati); 
    })